...
using InvertibleNetworks: get_params, set_params!, mse, ∇mse, ResidualBlock, FluxBlock
...
function test_logdet_per_batch()
    nx = 24
    ny = 24
    k = 4
    n_hidden = 4
    ...
    Y, lgdet_scalar = L.forward(X_batch; logdet_per_batch=false)
    @test lgdet_scalar isa Number

    # Test with logdet_per_batch=true (per-batch vector output)
    Y, lgdet_vector = L.forward(X_batch; logdet_per_batch=true)
    ...
    # Test invertibility with logdet_per_batch
    X_inv = L.inverse(Y)
    @test isapprox(norm(X_batch - X_inv)/norm(X_batch), 0f0; atol=1e-2)
end

test_logdet_per_batch()
...
function test_gradients_per_batch()
    nx = 24
    ny = 24
    k = 4
    n_hidden = 4
    ...
    function loss_per_batch(L, X, Y)
        Y_, logdet_vec = L.forward(X; logdet_per_batch=true)
        f = mse(Y_, Y) - sum(logdet_vec)/size(X)[end]
        ΔY = ∇mse(Y_, Y)
        ΔX = L.backward(ΔY, Y_)[1]
    ...
    batchsize = 4
    X_batch = randn(Float32, nx, ny, k, batchsize)
    X0_batch = randn(Float32, nx, ny, k, batchsize)
    dX_batch = X_batch - X0_batch

    hfactor=8f-1
    expected_f1 = 1f0 / hfactor
    expected_f2 = 1f0 / hfactor ^2f0

    Y = L.forward(X_batch)[1]
    f0, ΔX = loss_per_batch(L, X0_batch, Y)[1:2]
    h = 0.1f0
    maxiter = 6
    ...
    for j=1:maxiter
        f = loss_per_batch(L, X0_batch + h*dX_batch, Y)[1]
        err1_pb[j] = abs(f - f0)
        err2_pb[j] = abs(f - f0 - h*dot(dX_batch, ΔX))
        print(err1_pb[j], "; ", err2_pb[j], "\n")
        h = h * hfactor
    end

    factor1 = err1_pb[1:end-1]./err1_pb[2:end]
    factor2 = err2_pb[1:end-1]./err2_pb[2:end]

    @test isapprox(mean(factor1), expected_f1; atol=1f0)
    @test isapprox(mean(factor2), expected_f2; atol=1f0)
end

test_gradients_per_batch()
