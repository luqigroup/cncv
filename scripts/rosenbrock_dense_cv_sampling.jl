...
# Import forward from CNCV module
import CNCV: forward

font_prop = set_plot_configs()[1]
args = read_config("rosenbrock_dense_cv_sampling.json")
...
X_true = rand(RB_dist, 1)  # True parameter (2×1)
Y_obs = X_true + args["sigma"] * randn(Float32, 2, 1)  # Observed data (2×1)

println("True parameter X: ", X_true[:, 1])
println("Observed data Y: ", Y_obs[:, 1])
...
function posterior_energy(x, y_obs, sigma)
    # -log p(x|y) = -log p(y|x) - log p(x) + const
    # = (1/2σ²)||x-y||² - log p(x)
    # Note: logpdf expects 2D matrix (column vectors)
    likelihood_term = sum((x .- y_obs) .^ 2) / (2 * sigma^2)
    prior_term = -logpdf(RB_dist, reshape(x, 2, 1))[1]
    return likelihood_term + prior_term
end

# MCMC sampling
X_test = zeros(Float32, 2, test_size)
x_current = rand(RB_dist, 1)[:, 1]  # Initialize from prior (2-element vector)
step_size = 0.01f0
n_burnin = 1000
...
for i = 1:(test_size + n_burnin)
    ...
    grad_likelihood = (x_current .- Y_obs[:, 1]) ./ (args["sigma"]^2)
    grad_prior = -gradlogpdf(RB_dist, reshape(x_current, 2, 1))[:, 1]
    grad_energy = grad_likelihood + grad_prior
    ...
    energy_current = posterior_energy(x_current, Y_obs[:, 1], Float32(args["sigma"]))
    energy_proposal = posterior_energy(x_proposal, Y_obs[:, 1], Float32(args["sigma"]))
...
println("Observed data Y_obs: ", Y_obs[:, 1])
println("True parameter X_true: ", X_true[:, 1])
...
fig = figure("mse-x1", figsize = (8, 6))
loglog(sample_sizes, vanilla_mse_x1, "o-", color = "#d62728", linewidth = 2, markersize = 8, label = "Vanilla")
loglog(sample_sizes, cv_mse_x1, "s-", color = "#2ca02c", linewidth = 2, markersize = 8, label = "CV-corrected")
# Plot 1/n reference line
loglog(sample_sizes, vanilla_mse_x1[1] * sample_sizes[1] ./ sample_sizes, "--", color = "#bfbfbf", linewidth = 1.5, label = "∝ 1/n")
xlabel("Sample size n")
ylabel("MSE")
title("MSE for x₁ mean estimator")
legend()
grid(true, alpha = 0.3)
...
fig = figure("mse-x2", figsize = (8, 6))
loglog(sample_sizes, vanilla_mse_x2, "o-", color = "#d62728", linewidth = 2, markersize = 8, label = "Vanilla")
loglog(sample_sizes, cv_mse_x2, "s-", color = "#2ca02c", linewidth = 2, markersize = 8, label = "CV-corrected")
loglog(sample_sizes, vanilla_mse_x2[1] * sample_sizes[1] ./ sample_sizes, "--", color = "#bfbfbf", linewidth = 1.5, label = "∝ 1/n")
xlabel("Sample size n")
ylabel("MSE")
title("MSE for x₂ mean estimator")
legend()
...
# Plot std vs sample size for x1
fig = figure("std-x1", figsize = (8, 6))
loglog(sample_sizes, vanilla_std_x1, "o-", color = "#d62728", linewidth = 2, markersize = 8, label = "Vanilla")
loglog(sample_sizes, cv_std_x1, "s-", color = "#2ca02c", linewidth = 2, markersize = 8, label = "CV-corrected")
loglog(sample_sizes, vanilla_std_x1[1] * sqrt(sample_sizes[1]) ./ sqrt.(sample_sizes), "--", color = "#bfbfbf", linewidth = 1.5, label = "∝ 1/√n")
xlabel("Sample size n")
ylabel("Standard deviation")
title("Std for x₁ mean estimator")
legend()
grid(true, alpha = 0.3)
wsave(joinpath(save_path, "std-vs-n-x1.png"), fig)
close(fig)

# Plot std vs sample size for x2
fig = figure("std-x2", figsize = (8, 6))
loglog(sample_sizes, vanilla_std_x2, "o-", color = "#d62728", linewidth = 2, markersize = 8, label = "Vanilla")
loglog(sample_sizes, cv_std_x2, "s-", color = "#2ca02c", linewidth = 2, markersize = 8, label = "CV-corrected")
loglog(sample_sizes, vanilla_std_x2[1] * sqrt(sample_sizes[1]) ./ sqrt.(sample_sizes), "--", color = "#bfbfbf", linewidth = 1.5, label = "∝ 1/√n")
xlabel("Sample size n")
ylabel("Standard deviation")
title("Std for x₂ mean estimator")
legend()
...
variance_reduction_x1 = vanilla_mse_x1 ./ cv_mse_x1
variance_reduction_x2 = vanilla_mse_x2 ./ cv_mse_x2

fig = figure("variance-reduction", figsize = (10, 6))
semilogx(sample_sizes, variance_reduction_x1, "o-", color = "#1f77b4", linewidth = 2.5, markersize = 10, label = "x₁ component")
semilogx(sample_sizes, variance_reduction_x2, "s-", color = "#ff7f0e", linewidth = 2.5, markersize = 10, label = "x₂ component")
axhline(y = 1.0, color = "#7f7f7f", linestyle = "--", linewidth = 2, label = "No reduction")
xlabel("Sample size n")
ylabel("MSE ratio (Vanilla / CV)")
title("Variance Reduction Factor")
legend(fontsize = 12)
grid(true, alpha = 0.3)
ylim([0, maximum([variance_reduction_x1; variance_reduction_x2]) * 1.1])
...
println("Variance reduction factor (x1): ", mean(variance_reduction_x1))
println("Variance reduction factor (x2): ", mean(variance_reduction_x2))
