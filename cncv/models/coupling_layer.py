...
class DenseConditionalLayerCV_Reversible(nn.Module):
    ...
    Args:
        ...
        split_idx: Where to split input (default: n_in // 2)
    ...
    def __init__(
        ...
        # Use provided split_idx or default to midpoint
        if split_idx is None:
            self.split_idx = n_in // 2
        else:
            # Validate split_idx
            if split_idx < 1 or split_idx >= n_in:
                raise ValueError(f"split_idx must be between 1 and {n_in-1}, got {split_idx}")
